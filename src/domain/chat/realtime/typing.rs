...
/// Aggregate statistics across all typing sessions
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AggregateTypingStatistics {
    /// Total number of users with active typing states
    pub total_users: usize,
    /// Total typing events across all sessions
    pub total_typing_events: u64,
    /// Combined typing duration across all users
    pub total_typing_duration: Duration,
    /// System-wide typing indicator statistics
    pub indicator_stats: TypingStatistics,
}

/// Typing indicator manager with lock-free concurrent operations
pub struct TypingIndicator {
    /// Active typing states with zero-allocation key storage
    pub(crate) typing_states: Arc<SkipMap<String, Arc<TypingState>>>,
