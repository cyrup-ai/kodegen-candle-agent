// Tests extracted from src/domain/util/json_util.rs

use kodegen_candle_agent::domain::util::json_util::{
...
fn stringified_roundtrip() -> Result<(), Box<dyn std::error::Error>> {
    let original = DummyStringified {
        data: serde_json::json!({"k":"v"}),
    };
    let s = serde_json::to_string(&original)?;
    assert_eq!(s, r#"{"data":"{\"k\":\"v\"}"}"#);
    let parsed: DummyStringified = serde_json::from_str(&s)?;
